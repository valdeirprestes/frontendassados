...
function validarcampos(){
    ...
    "O usuário para ser registrado precisa ter o nome preenchido, mais algum outro campo para contato",
    -1);
        return false;
    ...
    if(tagdatamovimento.value.length< 1){
        criatelaDialogo("divflexvertical",
        "Coloco a dia do pedido em no campo data",
    -1);
        return false;
    }
    try {
    ...
    if(listaprodutos.length<1){
        criatelaDialogo("divflexvertical",
    "Para criar um produto é necessário pelo menos um produto ",
    -1);
        return false;
    }
...
async function gestaopedido_registrarnovopedido(){
    ...
    let mododeentrega = (tagmododeentrega.checked)? "COMENTREGA":"SEMENTREGA";
    let status_pagamento = (tagstatus_pagamento.checked)?"PAGO":"PENDENTE";
